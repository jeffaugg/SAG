...
export class UbsRepository implements IUbsRepository {
  ...
  async findAll({ skip, limit }: PaginacaoDto) {
    ...
    const [total, items] = await this.prisma.$transaction([
      ...
      this.prisma.uBS.findMany({ where, skip, take: limit, orderBy: { nome: 'asc' } }),
    ]);
  ...
  async update(id: string, data: CreateUbsDto) {
    return this.prisma.uBS.update({ where: { id }, data: { ...data, updatedAt: new Date() } });
  }

  async delete(id: string) {
    return this.prisma.uBS.update({ where: { id }, data: { deletedAt: new Date() } });
  }
